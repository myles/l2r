...
sub new {
	...
	$radio->{dx} = int(rand(10)) - 5;
	$radio->{dy} = int(rand(10)) - 5;
...
# 		send A message
sub clortho_reply {
	my($radio, $egon, $clortho) = @_;

	my($name) = $radio->{name};
	my($status) = $radio->{status};

	if ($status eq EGON && $name eq $clortho) {
		$radio->{status} = CLORTHO;
		$radio->{slaves}{$egon} = 10_000;
		$radio->transmit("A $name $egon");	# venkmann_acknowledge
		return;
	}
...
sub clortho_greeting {
	my($radio, $clortho, $uptime, $nslaves) = @_;

	my($name) = $radio->{name};
	my($status) = $radio->{status};

	return if $status ne CLORTHO;

	my($my_slaves) = $radio->have_slaves();
	...
	return if ($my_slaves + $nslaves) > 256;

	if ($my_slaves <= $nslaves) {
print "I be merging to $clortho\n";
		$radio->transmit("M $name $clortho .");
	} else {
print "Please be merging $clortho\n";
		$uptime = $name; $uptime =~ s/^u-//;
		$radio->transmit("G $name $uptime $my_slaves");
	}
...
sub merge_request {
	...
	if ($status ne CLORTHO) {
		print("M request, $name not CLORTHO\n");
		return;
	}
...
sub weak_zuul_announcement {
	...
	if ($status eq EGON or $status eq CLORTHO) {
		print "Weak zuul announcement to non VENKMANN\n";
		return;
	}

	if ($clortho eq $master) {
		print "Weak zuul announcemnet me=$name master=$master to=$to from=$from clortho=$clortho\n";
		return;
	}
	$radio->{status} = WEAKZUUL;
...
sub zuul_announcement {
	...
	if ($status ne CLORTHO) {
		print "zuul_announce to non CLORTHO\n";
		return;
	}
